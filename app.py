...
# On réinitialise les index pour l’affichage
 ...
 if st.session_state.page == "contact":
    ...
    if len(mdf) == 0:
        st.warning("Aucun message envoyés")
    else:
      mdf_athlete = mdf[mdf["athlete"] == athlete]
      if len(mdf_athlete) > 0:
       for _, row in mdf_athlete.iterrows():
          st.chat_message("user").write(row["message"])
          st.chat_message("assistant").write(row["reponse"])
      else:
       st.warning("Aucun message envoyés")
